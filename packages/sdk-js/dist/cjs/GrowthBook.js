...
class GrowthBook {
  ...
  initSync(options) {
    ...
    if (options.streaming) {
      ...
      (0, _featureRepository.startAutoRefresh)(this, true);
  ...
  async decryptPayload(data, decryptionKey, subtle) {
    ...
    if (data.encryptedFeatures) {
      data.features = JSON.parse(await (0, _util.decrypt)(data.encryptedFeatures, decryptionKey || this._ctx.decryptionKey, subtle));
      delete data.encryptedFeatures;
    }
    if (data.encryptedExperiments) {
      data.experiments = JSON.parse(await (0, _util.decrypt)(data.encryptedExperiments, decryptionKey || this._ctx.decryptionKey, subtle));
  ...
  _run(experiment, featureId) {
    ...
    if (this._ctx.stickyBucketService && !experiment.disableStickyBucketing) {
      const {
        ...
        key: attrKey,
        doc
      } = this._generateStickyBucketAssignmentDoc(hashAttribute, (0, _util.toString)(hashValue), {
  ...
  _getStickyBucketAssignments(expHashAttribute, expFallbackAttribute) {
    ...
    const hashKey = "".concat(hashAttribute, "||").concat((0, _util.toString)(hashValue));
    const {
      hashAttribute: fallbackAttribute,
      hashValue: fallbackValue
    } = this._getHashAttribute(expFallbackAttribute);
    const fallbackKey = fallbackValue ? "".concat(fallbackAttribute, "||").concat((0, _util.toString)(fallbackValue)) : null;
    const assignments = {};
